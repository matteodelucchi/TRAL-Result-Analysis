...
To get deeper insights about the proteins containing TRs, more information is added from SwissProt by a left join
...
In 243 genes associated with unfavorable CRC expressing 286 proteins we could *de novo* detect 617  TRs of which after filtering and clustering 100 TRs remained. In CRC favorable genes (352) 403 proteins are expressed, where we could *de novo* detect 486 TRs with 48 TRs beeing left after filtering and clustering.

Additionally, 644 proteins were associated in Swiss-Prot with the Wnt-pathway, where we could *de novo* detect 1353 TRs of which 192 TRs remained after filtering and clustering. 
...
```
Of all proteins expressed by genes associated for beeing favorable or unfavorable for CRC, 9.4% and 22.4% contain at least one TR respectively. Analogously, we could detect TRs in 138 (21.4%) of the 644 proteins involved in the Human Wnt-pathway.
...
Looking seperately at CRC favorable and unfavorable proteins it can be observed that favorable CRC proteins have a greater tendecy for N-terminal location of their homo-TRs. Which allows to hypothesize, that favorable CRC proteins have a signal sequence and may play extracellular roles.
```{r warning=FALSE}
TR_location(
...
The small dataset doesn't allow to draw any final conclusions, even if we combine all groups toghether. 
...
However, we can see, that Alanine is most frequent (17.5%) followed by Serine (15.6%) in CRC and Wnt-/$\beta$-catenin pathway associated proteins.
...
Plotting the amino acid frequency ratio of TRs in CRC and Wnt-/$\beta$-catenin pathway associated proteins against the amino acid disorderpropensity we can see that almost only amino acids with high disorder propensity are part of TR in CRC and Wnt-/$\beta$-catenin pathway associated proteins. With exception of Lysine and Histidine.
```{r}
AAratio_vs_Disorderpropensity(sp_overall = TRUE)
```

This trend is consistent with the overall amino acid frequency of all known proteins - amino acids with higher disorderpropensity are appear in TRs significantly more frequent.
...
In proteins associated with unfavorable CRC prognosis we found in general more proteins with $\geq 4$ TRs comüpared to the group of unfavorable or Wnt-pathway proteins.
...
```
And Forkhead-related transcription factor 3 (ID: Q12948) appears to have 5 TR-regions which is promoting cell growth inhibition and is involved in cell migration.
...
```
In all three groups of proteins we could only detect homo- and micro-TRs which are known to be disease causing.
...
```
All TR-regions have a mean number of repeat units of 7 units per repeat with a mean TR-region length of 8 amino acids in a range of 6 up to 27, 20 and 42 residues for CRC favorable, unfavorable and Wnt-pathway associated proteins respectively.
...
```
With the applied filtering approach, no TRs were found in any Casein kinase I isoform epsilon. Neither in the Wnt-pathway protein set nor in the CRC favorable or unfavorable set.

[TODO: Just a thought. But as GSK has no relevant TRs either, this might give a hint for rareness of TRs in Kinases in general?]
...
[Liu2000]: http://dx.doi.org/10.1038/79859
...
TODO: is this the region where it interacts with Frizzled? Look at Frizzled where it interacts with what
...
We couldn't detect in both of those Kinase any TRs as we couldn't in CKI and GSK3 neither. It is thus possible to hypothesise that protein kinases tend to have few TR in general which should be supported by more data though.
...
```
Of the 483 protein kinases from the Human proteome 11 have TRs and are associated with the Wnt-pathway and 7 have TRs and are CRC related proteins.
To put this in relations, of the whole human proteome 2.4% are protein kinases. From proteins with TRs, 4.7% belong to the enzyme class of protein kinases in CRC associated proteins and 5.7% in Wnt-pathway proteins. 
```{r}
